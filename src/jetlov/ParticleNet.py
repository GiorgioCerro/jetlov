...
from .util import segmented_knn_graph
...
class ParticleNet(nn.Module):
        ...
        **kwargs
        ...
        if self.use_fusion:
            ...
            self.fusion_block = nn.Sequential(
                nn.Linear(in_chn, out_chn), nn.ReLU(), nn.BatchNorm1d(out_chn)
            )
    ...
    def forward(self, batch_graph):
        features = batch_graph.ndata["features"]
        g = batch_graph
        ...
        for idx, (k, conv) in enumerate(zip(self.k_neighbors, self.edge_convs)):
            if idx > 0:
                g = dgl.remove_self_loop(segmented_knn_graph(fts, k + 1, segs)).to(
                    features.device
                )
            fts = conv(g, fts)
            if self.use_fusion:
