...
class DGLGraphDatasetParticle(Dataset):
    def __init__(self, filepath_bkg, filepath_sig, nev=-1, n_samples=1000):
        ...
        self.data = []
        self.label = []
        for constits in tqdm(reader_bkg):
            if len(self.data) >= (n_samples / 2):
                break
            self.data += [self._build_graph(constits)]
            self.label += [0]
        for constits in tqdm(reader_sig):
            if len(self.data) >= (n_samples):
                break
            self.data += [self._build_graph(constits)]
            self.label += [1]
        print(
            f""" ... Total time to read input files + construct the graphs
            for {len(self.label)} jets: {time.process_time() - tic} seconds"""
        )
        if dump_number_of_nodes:
            df = pd.DataFrame(
                {
    ...
    def _build_graph(self, constits):
        ...
        spatialCoord = np.stack(
            [delta_eta_reflect(constits_p4, jet_p4), constits_p4.delta_phi(jet_p4)],
            axis=1,
        )
...
def prepare_knn_graph(graph, k=16, min_nodes=20):
    nng = remove_self_loop(
        knn_graph(graph.ndata["coordinates"], min(graph.number_of_nodes(), k + 1))
    )
    nng.ndata["coordinates"] = graph.ndata["coordinates"]
    nng.ndata["features"] = graph.ndata["features"]
    if nng.number_of_nodes() < min_nodes:
        nng.add_nodes(min_nodes - nng.number_of_nodes())
        fts = pad_array(graph.ndata["features"], min_nodes, 0)
        assert nng.number_of_nodes() == fts.shape[0]
    return nng


class VicRegLundDataset(Dataset):
    ...
    def __init__(
        ...
        super(VicRegLundDataset, self).__init__()
        print(f"Start loading dataset {filepath_bkg} (bkg) and {filepath_sig} (sig)")
        tic = time.process_time()
        ...
        for jet_id, jet in tqdm(enumerate(reader_sig)):
            ...
            self.data += [self._build_tree(JetTree(jet))]
            self.label += [1]
            self.shower_id += [next(reader_sig_shower)]
        print(
            """ ... Total time to read input files + construct the graphs
            for {num} jets: {ts} seconds""".format(
                num=len(self.shower_id), ts=time.process_time() - tic
            )
        )
        # if dump_number_of_nodes:
        #    df = pd.DataFrame({'num_nodes': np.array(
...
class VicRegPartDataset(Dataset):
    def __init__(self, filepath_bkg, filepath_sig, nev=-1, n_samples=1000):
        super(VicRegPartDataset, self).__init__()
        print(f"Start loading dataset {filepath_bkg} (bkg) and {filepath_sig} (sig)")
        # tic = time.process_time()
        ...
        for _ in range(int(n_samples / 4)):
            self.data += [
                (
                    self._build_graph(next(reader_bkg)),
                    self._build_graph(next(reader_bkg)),
                )
            ]
            self.label += [0]
        for _ in range(int(n_samples / 4)):
            self.data += [
                (
                    self._build_graph(next(reader_sig)),
                    self._build_graph(next(reader_sig)),
                )
            ]
            self.label += [1]
