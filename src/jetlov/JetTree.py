...
class LundCoordinates:
    ...
    def __init__(self, j1, j2):
        ...
        self.lnm = np.float32(0.5 * math.log(abs((j1 + j2).m2()) + smooth))
        self.lnKt = np.float32(math.log(j2.pt() * delta + smooth))
        self.lnz = np.float32(math.log(z + smooth))
        self.lnDelta = np.float32(math.log(delta + smooth))
        self.lnKappa = np.float32(math.log(z * delta + smooth))
        try:
            self.psi = np.float32(
                math.atan((j1.rap() - j2.rap()) / (j1.phi() - j2.phi()))
            )
    ...
    def state(self):
        ...
        return np.array(
            [getattr(self, v) for v in LundCoordinates.components], dtype="float32"
        )

    # ----------------------------------------------------------------------
    def children_pmu(self, pmuform=True):
        # return children's pmu (px, py, pz, e) in order j1 and j2
        if pmuform:
            return np.array(
                [
                    self.j1.px(),
...
class JetTree:
    ...
    def __init__(self, pseudojet, child=None):
        ...
        self.node = np.array(
            [pseudojet.px(), pseudojet.py(), pseudojet.pz(), pseudojet.E()],
            dtype="float32",
        )
    ...
    def jet(self, pseudojet=False):
        ...
        if not pseudojet:
            ...
            raise ValueError(
                "JetTree: jet() with pseudojet return value not implemented."
            )
...
class LundImage:
    ...
    # ----------------------------------------------------------------------
    def __init__(self, xval=[0.0, 7.0], yval=[-3.0, 7.0], npxlx=50, npxly=None):
    ...
    def fill(self, tree, res):
        """Fill the res array recursively with the tree declusterings of the
        hard branch."""
...
class RSD:
    ...
    def __call__(self, jet, returnTree=False):
        """Apply the groomer after casting the jet to a JetTree, and return
        groomed momenta."""
