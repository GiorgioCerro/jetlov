...
use_fusion = False
input_dims = 5
modelL = LundNet(input_dims=input_dims, num_classes=2, 
...
use_fusion = False
model = LundNet(input_dims=input_dims, num_classes=2, 
                conv_params=conv_params, fc_params=fc_params, 
                use_fusion=use_fusion)
...
#state_dict = torch.load("../experiments/logs/jetlov-0.pt", map_location="cpu")
#model.load_state_dict(state_dict)
#model.eval()
#
#model00 = RegNet() 
...
#state_dict = torch.load("../experiments/logs/jetlov-reshower-0.pt", map_location="cpu")
#modelL.load_state_dict(state_dict)
#modelL.eval()
...
dataset = Dataset(Path(path + "/test_bkg-0.hdf5"), Path(path + "/test_top-0.hdf5"), 
        nev=-1, n_samples=nn)
...
ax[0].scatter(2 * np.arange(len(std[mask])), std[mask], c='#FFA650', 
        label=f"{name}")
ax[0].scatter(2 * np.arange(len(stdL[mask])), stdL[mask], c='#50AAFF', 
        label=f"{nameL}")
...
ax[1].scatter(2 * np.arange(len(std[~mask])), std[~mask], c='#FFA650', 
        label=f"{name}")
ax[1].scatter(2 * np.arange(len(stdL[~mask])), stdL[~mask], c='#50AAFF', 
        label=f"{nameL}")
...
ax[2].scatter(2 * np.arange(len(mean[mask])), mean[mask], c='#FFA650', 
        label=f"{name}")
ax[2].scatter(2 * np.arange(len(meanL[mask])), meanL[mask], c='#50AAFF', 
        label=f"{nameL}")
ax[2].axhline(y=np.mean(mean[mask]), color='#FFA650', linestyle='--')
ax[2].axhline(y=np.mean(meanL[mask]), color='#50AAFF', linestyle='--')


ax[3].scatter(2 * np.arange(len(mean[~mask])), mean[~mask], c='#FFA650', 
        label=f"{name}")
ax[3].scatter(2 * np.arange(len(meanL[~mask])), meanL[~mask], c='#50AAFF', 
        label=f"{nameL}")
