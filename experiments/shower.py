...
def training_loop(args, device, model, optim, scheduler, dataset, val_dataset):
    ...
    # evaluation
    scores_eval, val_loss, val_loss_MSE, val_loss_XEntropy = eval(
        args, device, model, val_dataset
    )
    ...
    with wandb.init(
        ...
        for epoch in range(args.epochs):
            ...
            model, valid_acc = training_loop(
                args, device, model, optim, scheduler, dataset, valid_dataset
            )
            if epoch == 0:
...
def test(args, model, test_dataset):
    with wandb.init(
        ...
        scores_test, test_loss, test_loss_MSE, test_loss_XEntropy = eval(
            args, device, model, test_dataset
        )
